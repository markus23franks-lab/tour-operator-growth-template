...
const cases=[
 ...
 {name:'pricing opportunity outranks trust and discovery gaps',actual:()=>build({market:{queries:rows([WIN,GAP,GAP,GAP])},discoveryOpportunity:null,pricing:{state:'PRICING_POWER_CANDIDATE',operator:{median:100},market:{median:140},deltaPct:40,research:{verified:4}},offerComparison:{state:'COMPARABLE_SET_VERIFIED'},trust:{state:'REPUTATION_GAP',target:{rating:4.5,reviews:120},market:{competitors:4,medianRating:4.8,medianReviews:600}},conversion:{state:'INSUFFICIENT_EVIDENCE'}}).primary.sense,expected:'PRICING'},
 ...
 {name:'adjacent offer pattern survives as strategy without becoming fake opportunity',actual:()=>{const r=build({market:{queries:rows([WIN,WIN,WIN])},...baseline,productArchitecture:{state:'ADJACENT_OFFER_PATTERN',headline:'Adjacent offers repeat',action:'Investigate fit'}});return r.strategicContext.some(x=>x.type==='PRODUCT_ARCHITECTURE')&&r.opportunities.length===0},expected:true},
