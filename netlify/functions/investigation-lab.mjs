import {normalizeSerpEvidence,buildInvestigationSignals,buildResearchCoverage,buildEntityIntegrityPlan} from "./lib/investigation-core.mjs";
...
import {buildOperatorActionPlan} from "./lib/claim-ledger.mjs";
const LAB_BUILD_ID = "GO-INVESTIGATION-LAB-V0.11";
...
export default async (request) => {
  ...
  if(body.action==="run-proof"){
    ...
    const modelUsage=[];let diagnosticEvidence=[],diagnosticCoverage=null;
    ...
    try{
      ...
      diagnosticEvidence=finalRecords;diagnosticCoverage=coverage;
      ...
      const surfaceStatus=[...marketBatches,...followUp.batches].map(x=>({query:x.query,status:x.collected?.surfaceStatus||null,errors:x.collected?.errors||[x.error].filter(Boolean)}));
      ...
      return json(200,{ok:true,buildId:LAB_BUILD_ID,state:"PROOF_JUDGED",website:firstParty.website,capturedAt:new Date().toISOString(),dossier:dossierModel.dossier,initialPlan:initialPlanModel.plan,queriesResearched:plannedQueries,followUpQueries:followUp.attempted,surfaceStatus,signals:finalSignals,entityIntegrity,coverage,followUpPlan:followUpModel.plan,judgment:synthesisModel.synthesis,claimLedger:synthesisModel.claimLedger,actionPlan:buildOperatorActionPlan({ledger:synthesisModel.claimLedger,synthesis:synthesisModel.synthesis}),evidence:finalRecords,telemetry:{firstPartyPages:firstParty.pagesRead,competitorsRead:competitorEvidence.competitorsRead,evidenceRecords:finalRecords.length,initialQueries:plannedQueries.length,followUpQueries:followUp.attempted.length,discardedQuotes:synthesisModel.discardedQuotes,model:modelUsage},validation});
    }catch(error){return json(502,{ok:false,buildId:LAB_BUILD_ID,error:error instanceof Error?error.message:String(error),...(diagnosticEvidence.length?{evidence:diagnosticEvidence,coverage:diagnosticCoverage}:{}),...(error?.draftDossier?{draftDossier:error.draftDossier}:{}),...(error?.draftJudgment?{draftJudgment:error.draftJudgment}:{}),telemetry:{discardedQuotes:error?.discardedQuotes||[],model:modelUsage}})}
  }
