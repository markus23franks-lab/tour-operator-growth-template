...
The first claim-provenance step now attaches exact `supportQuotes` to model findings and checks action-ready quotes against the exact first-party detail text shown to the model. It has passed local regressions but not a live model run. The next technical milestone is a **product-scoped** claim ledger: typed relationships for product, price, booking destination, contact owner and domain identity, plus checks that a quoted passage actually supports the proposed interpretation. One bounded pilot should validate the quotation contract before more corpus spending. Founder evaluation should judge whether this cautious, source-first reasoning direction is worth integrating later, rather than treating the current recommendations as instructions to operators.
