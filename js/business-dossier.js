...
function productRows(ctx={}){
 const raw=[...(ctx.products||[]),...(ctx.offers||[])],seen=new Set(),out=[];
 for(const item of raw){const name=clean(typeof item==='string'?item:item?.name||item?.label);if(!name)continue;const k=name.toLowerCase();if(seen.has(k))continue;seen.add(k);out.push({name,transactionType:clean(item?.transactionType||transaction(ctx)),family:clean(item?.family||item?.activity||''),price:item?.price||null,duration:item?.duration||null,evidence:item?.evidence||'first-party public page'});}
