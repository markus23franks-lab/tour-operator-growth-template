...
function sourceText(ctx){const raw=String(ctx?.combined||'');const owned=raw.split(/(?<=[.!?])\s+|\n+/).filter(s=>!/(?:things? (?:to do|visitors? can find)|can find elsewhere|available elsewhere|nearby attractions?|other local (?:activities|experiences)|area also offers)/i.test(s)).join(' ');return [owned,(ctx?.commercialTruth?.primaryProducts||[]).map(x=>x.name||x).join(' '),(ctx?.commercialTruth?.segments||[]).map(x=>x.name||x).join(' ')].filter(Boolean).join('\n');}
function structuredText(ctx){return [(ctx?.commercialTruth?.primaryProducts||[]).map(x=>x.name||x).join(' '),(ctx?.commercialTruth?.segments||[]).map(x=>x.name||x).join(' '),(ctx?.siteArchitecture?.commercialPages||[]).map(x=>x.name||'').join(' '),(ctx?.siteArchitecture?.internalProducts||[]).map(x=>x.name||'').join(' ')].filter(Boolean).join('\n');}
