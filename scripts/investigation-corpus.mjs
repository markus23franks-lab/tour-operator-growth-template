...
// Usage: node scripts/investigation-corpus.mjs <function-url> <websites.json> <output.json>
import {readFile,writeFile} from 'node:fs/promises';

const [endpoint,inputPath,outputPath]=process.argv.slice(2);
if(!endpoint||!inputPath||!outputPath){console.error('Usage: node scripts/investigation-corpus.mjs <function-url> <websites.json> <output.json>');process.exit(2)}
const url=new URL(endpoint);
if(url.protocol!=='https:'&&url.hostname!=='localhost'&&url.hostname!=='127.0.0.1')throw new Error('The function endpoint must use HTTPS outside localhost');
...
for(const website of websites){
  ...
  try{
    const res=await fetch(url,{method:'POST',headers:{'Content-Type':'application/json',...(process.env.GO_LAB_TOKEN?{'Authorization':'Bearer '+process.env.GO_LAB_TOKEN}:{})},body:JSON.stringify({action:'run-proof',website}),signal:AbortSignal.timeout(180000)});
  ...
  await writeFile(outputPath,JSON.stringify({startedAt,endpoint:url.origin+url.pathname,results},null,2)+'\n',{mode:0o600});
}
