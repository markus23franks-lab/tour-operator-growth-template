(function(global){
  ...
  function buildJudgment(market, website){
    ...
    const failures = rows.filter(r => r.providerError).length;
    const visibleRows = rows.filter(r => finite(r.targetLocalPosition) || finite(r.targetOrganicPosition));
    const localVisible = rows.filter(r => finite(r.targetLocalPosition));
    ...
    const localRatio = rows.length ? localVisible.length/rows.length : 0;
    const organicRatio = rows.length ? organicVisible.length/rows.length : 0;
    ...
    let summary = '';
    if (!rows.length){
      constraint='Insufficient evidence'; headline='GO does not yet have enough market evidence to make a growth call.';
      summary='The evidence provider returned no commercial search observations, so GO should not manufacture a conclusion.';
      ...
      summary=`Across ${rows.length} commercial searches tied to this operator, GO observed the business in ${visibleRows.length}. The clearest gap is “${primary?.query || 'the highest-priority search'},” where the operator was ${primary ? formatPosition(primary) : 'not consistently visible'}.`;
    } else if (trustGap){
      constraint='Public trust advantage';
      headline='Discovery exists, but competitor trust may be doing more of the selling.';
      summary=`The operator is visible in ${visibleRows.length} of ${rows.length} searches, so pure visibility is not the only issue. A repeated competitor carries materially more public review proof, which may matter more than simply creating additional search exposure.`;
    } else if (top3.length >= Math.ceil(rows.length*.6)){
      constraint='No dominant search constraint';
      headline='Search visibility does not look like the obvious first problem.';
      summary=`GO observed top-three visibility in ${top3.length} of ${rows.length} commercial searches. That is counter-evidence against making SEO/search the default mission; GO should investigate conversion, trust, distribution or operations before prescribing more visibility work.`;
    } else {
      constraint='Selective discovery gap';
      headline='The discovery weakness is selective, not universal.';
      summary=`The operator is visible in ${visibleRows.length} of ${rows.length} tested searches, but the evidence is uneven. GO should attack the highest-commercial-intent gap instead of treating the entire site as an SEO problem.`;
    }

    const facts=[];
    facts.push(`GO tested ${rows.length} commercial searches and observed the operator in ${visibleRows.length}; Local visibility appeared in ${localVisible.length} and organic visibility in ${organicVisible.length}.`);
    ...
    const counter=[];
    if (visibleRows.length) counter.push(`The operator was visible in ${visibleRows.length} of ${rows.length} tested searches, so this is not evidence of zero search presence.`);
    ...
    if (failures) counter.push(`${failures} provider request${failures===1?'':'s'} returned warnings, reducing certainty.`);
    ...
    if (rows.length>=4 && failures===0 && (visibilityRatio<=.4 || top3.length>=Math.ceil(rows.length*.6) || trustGap)) confidence='HIGH';
    if (failures >= Math.ceil(rows.length/2) || rows.length<3) confidence='LOW';

    let action='Investigate the highest-value discovery gap before changing the entire website.';
    if (primary){
    ...
    if (constraint==='No dominant search constraint') action='Do not create a search mission yet. Use this as evidence that GO should investigate conversion, trust, marketplace dependence and connected booking data before deciding what to work on first.';
    ...
    return {
      ...
      diagnostics:{queries:rows.length,visible:visibleRows.length,localVisible:localVisible.length,organicVisible:organicVisible.length,top3:top3.length,providerFailures:failures,visibilityRatio:Number(visibilityRatio.toFixed(2))}
    };
  }
