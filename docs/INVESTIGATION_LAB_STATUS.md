...
## September 24 product-scoped claim-ledger checkpoint

The quote contract is now followed by a product-scoped claim ledger. Each commercial finding is emitted as a typed claim carrying product labels, cited pages and domains, parsed amounts, booking links, operator contact emails, exact support quotes and provenance (`hasDetailPage`, `verifiedQuoteCount`). The ledger validator rejects missing source scope, unverifiable quotes and action-ready claims without both a first-party detail page and a verified quote. A deterministic `buildOperatorActionPlan` bridge converts valid ledger claims into GO's existing action-plan shape; anything lacking action provenance is downgraded to `INVESTIGATE` rather than presented as a validated opportunity.

The ledger and bridge pass focused and full local regression suites. The archived First Lady replay remains the trusted quote validation (`35950862567`); no new SerpApi searches or paid corpus run is required for this milestone. The ledger improves source truth and product linkage, but it does not establish causality, conversion, margin or ROI. Those require connected operating data and bounded product experiments.

The bridge is now connected to the real dashboard path: `PROOF_JUDGED` results expose an action plan, the Lab page persists the result to `growthOperatorResearchJudgment`, and `dashboard.html` renders the model-led current read and mission from that ledger-backed plan. Claims without action provenance remain investigations in the dashboard. This is the smallest credible product integration; the next founder judgment is the operator experience, not another Lab-only regression.
