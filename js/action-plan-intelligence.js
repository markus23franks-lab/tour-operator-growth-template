...
function build({dossier,brain,offerComparison,bookingJourney,productArchitecture,competition,positioningComparison}={}){
 ...
 if(offerComparison?.state==='COMPARABLE_SET_VERIFIED')push('PRICING','INVESTIGATE',offerComparison.headline,'GO verified a structured like-for-like public offer set.',offerComparison.action,'Connect booking volume, margin, capacity and demand response before changing price.','HIGH');
 ...
 return {version:'GO-ACTION-PLAN-V1',state:first?'READY':'KEEP_RESEARCHING',headline:first?first.headline:'GO has not found a business move strong enough to recommend yet.',moves:ranked,next:first?first.action:'Keep investigating until public evidence changes a business decision.',evidenceNote:'GO separates validated opportunities, investigations, assets to leverage and areas that require connected measurement. An investigation is not presented as a proven defect.'};
}
