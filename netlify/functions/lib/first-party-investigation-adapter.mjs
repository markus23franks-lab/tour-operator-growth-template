...
const contactEmails=text=>[...new Set(String(text).match(/[\w.+-]+@[\w.-]+\.[a-z]{2,}/gi)||[])].slice(0,20);
...
async function collectWithFirecrawl(website,apiKey,timeoutMs){
 ...
 try{
  ...
  const record={id:"ev_fp_"+hash(website+"|firecrawl|"+title),surface:"FIRST_PARTY_RENDERED",claimType:"FIRST_PARTY_HOME",subject:{entityId:null,label:title},observation:{url:website,title,headings:markdown.split("\n").filter(x=>/^#{1,3}\s/.test(x)).map(x=>clean(x.replace(/^#{1,3}\s+/,""))).slice(0,30),bookingLinks:(data.links||[]).filter(x=>/^https?:/i.test(String(x))&&/book|reserve|availability|ticket|rent|peek|fareharbor|bokun|rezdy|xola/i.test(String(x))).slice(0,20).map(url=>({label:"booking link",url})),contactEmails:contactEmails(markdown),prices:extractAmounts(markdown),text:clean(markdown).slice(0,30000),mainText:clean(markdown).slice(0,30000),screenshot:data.screenshot||""},source:{provider:"Firecrawl",url:website,providerRef:data.metadata?.sourceURL||""},observedAt,confidence:"HIGH",status:"OBSERVED"};
...
function htmlToObservation(html,url){
  ...
  const bookingLinks=[];for(const m of (main||html).matchAll(/<a\b[^>]*href=["']([^"']+)["'][^>]*>([\s\S]*?)<\/a>/gi)){const label=clean(decode(m[2].replace(/<[^>]+>/g," ")));if(/book|reserve|availability|ticket|rent/i.test(label)||/peek|fareharbor|bokun|rezdy|xola|checkfront|bookeo|rezgo|rocketrez/i.test(m[1])){try{const link=new URL(decode(m[1]),url);if(["http:","https:"].includes(link.protocol))bookingLinks.push({label,url:link.href})}catch{}}}
  const prices=extractAmounts(decode(html.replace(/<[^>]+>/g," ")));
  const plain=markup=>clean(decode(markup.replace(/<script[\s\S]*?<\/script>/gi," ").replace(/<style[\s\S]*?<\/style>/gi," ").replace(/<svg[\s\S]*?<\/svg>/gi," ").replace(/<[^>]+>/g," ")));
  const text=plain(html).slice(0,30000);
  const mainText=main&&plain(main).length>=80?plain(main).slice(0,30000):text;
  return {url,title,headings,bookingLinks:dedupe(bookingLinks,x=>x.url),contactEmails:contactEmails(text),prices,text,mainText};
}
