...
function buildGenericFindings(primary) {
  ...
  while (base.length < 5) base.push({ pillar: "Growth", title: "Create a measurable improvement rhythm", summary: "Work one priority at a time and measure the outcome.", found: "The business has multiple possible improvements but needs a clear order.", why: "Scattered effort makes it difficult to know what is producing results.", recommendation: "Choose one mission, define the expected result, and review the outcome before moving on.", expected: "More consistent execution and clearer proof of what drives growth.", status: "Opportunity", priorityScore: 70, confidence: 86 });
  return base.map((item, index) => ({
    ...
    recommendation: item.recommendation || "Address this opportunity with one focused improvement.", expected: item.expected || "A clearer customer journey and stronger measurable results.",
