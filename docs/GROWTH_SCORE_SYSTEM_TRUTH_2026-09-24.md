...
The public Analyzer previously computed a six-system /100 from website heuristics (including a fixed Intelligence value) and saved it in the prospect profile, despite withholding it in the visible Snapshot. New prospect and Cayman benchmark profiles now persist `null` for the Growth Score and six-system scores; the website findings remain separate. The Cayman benchmark is labeled as an example on both Analyzer and Snapshot. This removes a latent source of invented precision without changing which public observations become findings.

Mission and measurement summaries now follow operator-reported progress. A reported approved action overrides the earlier “approval pending” or “no approved execution” label. Dashboard and Growth Score expose the reported action date/person and baseline/follow-up values, units, dates and sources. Execution remains unverified and a before/after observation is not attributed to GO. The Growth-system health stays unknown.
