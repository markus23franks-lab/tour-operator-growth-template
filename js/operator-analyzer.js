...
async function runAnalysis(rawUrl) {
  ...
  try {
    ...
    if (token !== scanToken) return;
    const research = await runOperatorResearch(websiteContext, market, acquisition).catch(error => { console.warn('GO operator research incomplete', error); return {pricing:null,trust:null,conversion:null,brain:null,status:'PARTIAL'}; });
    if (token !== scanToken) return;
    ...
    const profile = buildUniversalProfile(url, [home, ...extraPages], market, bookingLinkEvidence, research);
    await wait(240);
...
async function requestResearchMarket(ctx, queries, lens) {
...
async function runOperatorResearch(ctx, discoveryMarket, acquisition) {
  const pricingPlan=window.GOPricingIntelligence?.plan?.({businessName:ctx.businessName,location:ctx.businessContext?.location||'',websiteQueries:(discoveryMarket?.queries||[]).map(q=>typeof q==='string'?q:q?.query).filter(Boolean)})||[];
  const trustPlan=window.GOTrustIntelligence?.plan?.({businessName:ctx.businessName,location:ctx.businessContext?.location||''})||[];
  const results=await Promise.all([requestResearchMarket(ctx,pricingPlan,'pricing').catch(()=>null),requestResearchMarket(ctx,trustPlan,'trust').catch(()=>null)]);
  const pricing=window.GOPricingIntelligence?.build?.({pricingMarket:results[0],businessName:ctx.businessName,website:ctx.url})||null;
  const trust=window.GOTrustIntelligence?.build?.({trustMarket:results[1],businessName:ctx.businessName,website:ctx.url})||null;
  const conversion=window.GOConversionIntelligence?.build?.(acquisition)||null;
  const discoveryOpportunity=buildMarketFindings({businessName:ctx.businessName,url:ctx.url,offers:ctx.offers||[],prices:extractPrices(ctx.combined||''),businessContext:ctx.businessContext||{},market:discoveryMarket})?.find?.(x=>x.pillar==='Visibility')||null;
  const brain=window.GOOpportunityBrain?.build?.({market:discoveryMarket,discoveryOpportunity,pricing,trust,conversion})||null;
  return {version:'GO-OPERATOR-RESEARCH-V1',status:brain?'COMPLETE':'PARTIAL',pricing,trust,conversion,brain,researchPlans:{pricing:pricingPlan,trust:trustPlan}};
}
function researchCandidateToFinding(c,market){
  if(!c||!c.sense)return null;
  const map={DISCOVERY:['Visibility','⌖'],PRICING:['Growth','
  const combined = `${pages.map(page => page.markdown).join("\n\n")}\n\n${bookingLinkEvidence}`;
  const home = pages[0]?.markdown || combined;
  ...
  const opportunities = mergeResearchJudgment(research, heuristicOpportunities, market);
  ...
  return {
    ...
    pipelineDebug: {
      research: research?.brain ? { status: research.status || 'COMPLETE', headline: research.brain.headline, primary: research.brain.primary, candidates: research.brain.candidates } : { status: research?.status || 'UNAVAILABLE' },
...
function escapeHtml(value) { return String(value ?? "").replace(/[&<>'"]/g, char => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", "'": "&#39;", '"': "&quot;" }[char])); }],TRUST:['Trust','★'],CONVERSION:['Conversion','↗']};
  const pair=map[c.sense]||['Intelligence','◎'],pillar=pair[0],icon=pair[1],state=c.state||'UNKNOWN',actionable=state==='OPPORTUNITY',healthy=/^HEALTHY/.test(state)||state==='FOUNDATION_OBSERVED';
  const checked=c.sense==='DISCOVERY'?(market?.queryResults||[]).map(row=>({query:row.query,demandLabel:'representative commercial search',localPosition:row.targetLocalPosition??null,organicPosition:row.targetOrganicPosition??null,localStatus:row.targetLocalPosition?'Maps #'+row.targetLocalPosition:(row.evidenceState==='OBSERVED_GAP'?'Not found in checked Maps results.':'Maps evidence unknown.'),organicStatus:row.targetOrganicPosition?'Organic #'+row.targetOrganicPosition:(row.evidenceState==='OBSERVED_GAP'?'Not found in checked regular results.':'Organic evidence unknown.')})):undefined;
  const why=c.sense==='PRICING'?'Pricing changes revenue per booking, so a validated move can matter without buying more traffic.':c.sense==='TRUST'?'Reputation changes how much confidence travelers bring into the booking decision.':c.sense==='CONVERSION'?'Improving existing demand can be more valuable than paying to acquire more of it.':'Discovery matters when missing demand is commercially relevant and can become qualified visits and bookings.';
  const metric=c.sense==='PRICING'?'Price / product mix → booking conversion → revenue per booking':c.sense==='TRUST'?'Review position + trust exposure → booking conversion':c.sense==='CONVERSION'?'Website visit → booking start → completed booking':'Qualified visibility → site visits → bookings';
  return {kind:actionable?'opportunity':'investigation',pillar,icon,title:c.finding,problem:actionable?c.finding+' This survived GO’s cross-signal judgment as an opportunity worth validating.':healthy?c.finding+' GO treats this as working evidence, not a defect.':c.finding+' GO does not have enough evidence to turn this into work yet.',whyItMatters:why,action:c.action,metric,moneyLabel:c.economicImportance==='POTENTIALLY_HIGH'?'Economic upside needs connected volume + margin':'Revenue needs connected data',confidence:String(c.confidence||'MEDIUM').toLowerCase().replace(/^./,x=>x.toUpperCase()),priorityReason:actionable?'Multiple evidence checks support a commercially relevant issue; GO still requires the stated proof before execution.':healthy?'This area appears to be working, so GO should leverage or measure it rather than manufacture a fix.':'GO keeps this below a mission until the missing evidence is recovered.',counterEvidence:c.requiredNextEvidence?'This judgment can change when GO gets: '+c.requiredNextEvidence+'.':'Connected business evidence could change the priority.',checkedSearches:checked,searchSelectionWhy:c.sense==='DISCOVERY'?'GO selected a small portfolio from verified product families and destination context; unrelated destination activities are excluded.':'',sources:[{type:'public',label:'GO research',detail:c.finding},{type:'operator',label:'GO judgment',detail:'State: '+state+'. Evidence strength: '+(c.evidenceStrength||'unknown')+'.'}],priorityScore:actionable?20:healthy?10:6};
}
function mergeResearchJudgment(research,heuristics,market){
  const brain=research?.brain;if(!brain?.candidates?.length)return heuristics;
  const ordered=[];
  if(brain.primary){const first=researchCandidateToFinding(brain.primary,market);if(first)ordered.push(first);}
  for(const candidate of brain.candidates){if(ordered.some(x=>x.title===candidate.finding))continue;const item=researchCandidateToFinding(candidate,market);if(item)ordered.push(item);}
  const pillars=new Set(ordered.map(x=>x.pillar));const extra=(heuristics||[]).find(x=>!pillars.has(x.pillar)&&x.priorityScore>=9);if(extra)ordered.push(extra);
