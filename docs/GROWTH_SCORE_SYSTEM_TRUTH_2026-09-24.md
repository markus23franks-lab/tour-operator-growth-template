...
The system-truth regression covers cited concern and strength, uncited rejection, unknown systems, Mission baseline/action/follow-up, and operator/run isolation. The dashboard-entry regression covers default empty state, preservation of researched mode, and the explicit demo engine. All 36 local regressions pass. The cloud browser rejected the locally served page with `ERR_BLOCKED_BY_CLIENT`, and no accessible staging URL exists, so visual and interaction browser validation remain a release gate before a founder-facing operator test.
