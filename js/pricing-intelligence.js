(()=>{
 ...
 function build({pricingMarket,businessName,website}){
   const {own,comp}=collect(pricingMarket,{businessName,website}),ov=[...new Set(own.map(x=>x.value))],cv=comp.map(x=>x.value),om=median(ov),cm=median(cv),sources=[...new Set(comp.map(x=>host(x.link)||norm(x.name)).filter(Boolean))];
   ...
   if(om&&cm&&comp.length>=3&&sources.length>=2){delta=Math.round((cm-om)/om*100);if(delta>=10){state='PRICING_POWER_CANDIDATE';headline=`GO found a possible pricing-power gap: comparable public signals are about ${delta}% above the recovered operator price.`;recommendation='Verify that the products match on format, duration and inclusions. If they do, this deserves a controlled pricing mission.'}else if(delta<=-10){state='PREMIUM_POSITION';headline=`GO found the operator priced about ${Math.abs(delta)}% above the comparable public signal.`;recommendation='Do not discount automatically. Validate conversion and trust strength to determine whether the premium is working.'}else{state='MARKET_ALIGNED';headline='GO found the recovered operator price broadly aligned with comparable public pricing.';recommendation='Pricing does not currently outrank other growth investigations on public evidence alone.'}}
   return {version:'GO-PRICING-INTELLIGENCE-V3',state,headline,recommendation,deltaPct:delta,operator:{signals:own.length,median:om,sample:own.slice(0,6)},market:{signals:comp.length,sources:sources.length,median:cm,sample:comp.slice(0,8)},research:{planned:pricingMarket?.queries?.length||0,verified:(pricingMarket?.queries||[]).filter(x=>x.evidenceState!=='UNKNOWN').length},evidenceNote:'GO ran dedicated pricing searches. Public prices remain directional until product format, duration, inclusions and group structure are matched apples-to-apples. GO requires at least three market price signals from two distinct sources before even promoting a pricing-power candidate.'};
 }
