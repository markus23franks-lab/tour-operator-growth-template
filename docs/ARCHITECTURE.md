...
The first broader-product slice after the research-to-dashboard bridge records a **real operator-supplied baseline, reported action and later measurement** against a researched claim. `js/research-scope.js` gives saved checklist progress, the saved investigation and the Outcome record one shared identity: website origin, claim ID, cited evidence IDs, headline, normalized cited passages and investigation capture timestamp. `js/mission-outcomes.js` verifies the full identity on read (including against a storage hash collision), validates nonnegative values, dates, source, metric unit and matching period labels, locks the baseline after action and requires an operator approval/performance attestation. The Mission exposes this as an optional Outcome record and the research dashboard reads back the same scoped state. A rerun with the same claim ID but a different capture timestamp or passage starts fresh. Older saved investigation records without this fingerprint do not match new research; older outcome records remain locally stored but do not match the new keys. An observed numerical difference is displayed as an observation. The action is `OPERATOR_REPORTED`, never `GO_EXECUTED`, and the result is not attributed revenue.
...
### Private research artifact → dashboard handoff

The private Actions evaluator runs the paid investigation; a public Netlify function is not its substitute. `Pages/investigation-lab.html` now accepts one **completed, reviewed, single-operator** JSON evaluation artifact through a local file control. `js/research-artifact-import.js` checks successful judgment, claim/move/evidence ID agreement, source passages against cited evidence records, and action-ready detail-page provenance before saving a reduced dashboard read. The raw artifact, provider telemetry and evidence collection are not uploaded or stored by this flow. The dashboard and Mission label this read `ARCHIVED EVALUATION` with the run date. Direct Lab results carry a `LIVE_LAB` capture date; old undated reads are labeled `DATE UNKNOWN`.

This browser-local handoff is not authentication, cryptographic attestation, a shared account, fresh research, or a durable customer record. A JSON file can itself be edited; a Builder must inspect the private run before offering it for founder review. The Lab live-run button still requires backend authorization and provider quota; do not put OpenAI or SerpApi secrets into the public site. A legitimate staged browser run remains the founder release gate.
