...
async function runDedicatedOperatorResearch(ctx, market, acquisition) {
  ...
  const brain=window.GOOpportunityBrain?.build?.({market,discoveryOpportunity,pricing,offerComparison,trust,conversion,bookingJourney,productArchitecture,competition,positioningComparison})||null;
  const actionPlan=window.GOActionPlan?.build?.({dossier,brain,offerComparison,bookingJourney,productArchitecture,competition,positioningComparison})||null;
  const presentationGate=window.GOPresentationGate?.inspect?.({dossier,brain,actionPlan})||{pass:true,issues:[]};
  return {version:'GO-OPERATOR-RESEARCH-V11',dossier,pricing,offerComparison,trust,conversion,bookingJourney,productArchitecture,competition,positioning,positioningComparison,brain,actionPlan,presentationGate,researchPlans:{discovery:discoveryQueries.slice(0,5),pricing:pricingPlan,trust:trustPlan},evidence:{pricingMarket:pricingMarket?{provider:pricingMarket.provider||'SerpApi',observedAt:pricingMarket.observedAt||'',queries:pricingMarket.queries||[]}:null,trustMarket:trustMarket?{provider:trustMarket.provider||'SerpApi',observedAt:trustMarket.observedAt||'',queries:trustMarket.queries||[],target:trustMarket.target||null}:null}};
}
function brainFinding(c) {
  if(!c)return null;
  const meta={DISCOVERY:['Visibility','search visibility to qualified visits and bookings'],PRICING:['Growth','price and product mix to revenue per booking'],TRUST:['Trust','reputation and trust exposure to booking conversion'],CONVERSION:['Conversion','website visit to booking start to completed booking']}[c.sense]||['Intelligence','validated signal to revenue'];
